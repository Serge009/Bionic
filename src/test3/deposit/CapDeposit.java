...
/**
 * deposit type 2 - with capitalization
 */
public class CapDeposit extends AbstractDeposit {
    ...
    private void monthsCalculation(long startDays, long endDays, long monthsBetween, double daysInStartPeriod, double daysInEndPeriod){
        ...
        this.sum += this.sum * (this.rate) * (startDays/daysInYear1) / 100.0;

        //calculating for each month between startDate and maturityDate
        for(int i = 0; i < monthsBetween; i++){
            tmp = tmp.plusMonths(1);
            daysInYear = tmp.isLeapYear() ? 366 : 365;
            this.sum += this.sum * this.rate * tmp.getMonth().length(tmp.isLeapYear()) / 100.0 / daysInYear;
        }

        //calculating for maturity month
        this.sum += this.sum * (this.rate ) * (endDays/daysInYear2) / 100.0;
    ...
    private void simpleCalculation(){
        ...
        if(startDate.getYear() == maturityDate.getYear()){
            this.interest = this.sum * (this.rate / 100.0) * (this.duration / daysInYear1);
        } else {
